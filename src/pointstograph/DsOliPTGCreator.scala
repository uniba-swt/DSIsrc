...
trait IDsOliPTGCreator {
  def createPTGs(): DsOliPointsToGraphs
}
...
class DsOliPTGCreator(val events: DsOliEvents, val typeDB: ITypeDB) extends IDsOliPTGCreator {
  ...
  def findVertexInRange(nodes: Graph[pointstograph.DsOliVertex, pointstograph.DsOliDiEdge]#NodeSetT, vertexStartAddress: Long, vertexEndAddress: Long): Option[DsOliVertex] = {
    ...
    DsOliLogger.debug(funSignature + "entered: ")
    var foundVertex: Option[DsOliVertex] = None
    breakable {
   ...
   */
  def createNewVertex(vertexType: DsOliType, vertexStartAddress: Long, vertexEndAddress: Long): DsOliVertexMemory = {
    val funSignature = classSignature + "createNewVertex: "
    DsOliLogger.debug(funSignature + "entered: ")
    return new DsOliVertexMemory(vertexStartAddress, vertexEndAddress, vertexType, getId())
  }
  ...
  def cleanupEdgesAndVertices(newVertex: DsOliVertexMemory, graph: DsOliGraph): Unit = {
    ...
    for (node <- vertices.iterator) {
      ...
      vertex match {
        ...
        case _ => // Nothing
          DsOliLogger.debug(funSignature + "Found something else than a memory vertex." + vertex)
      }
    }
  ...
  def addNewVertexWithCleanup(vertexStartAddress: Long, vertexEndAddress: Long, vertexType: DsOliType, graph: DsOliGraph): DsOliVertex = {
    val funSignature = classSignature + "addNewVertexWithCleanup: "
    val newVertex = createNewVertex(vertexType, vertexStartAddress, vertexEndAddress)
   ...
   */
  def findOrAddVertex(vertexStartAddress: Long, vertexType: DsOliType, graph: DsOliGraph): DsOliVertex = {
    val funSignature = classSignature + "findOrAddVertex: "
    DsOliLogger.debug(funSignature + "entered. vertexStartAddress: " + vertexStartAddress + ", vertexType: " + vertexType)
    val vertices = graph.graph.nodes

    val vertexEndAddress = vertexStartAddress + vertexType.size - 1

    return findVertexInRange(vertices, vertexStartAddress, vertexEndAddress) match {
      case Some(v) =>
        DsOliLogger.debug(funSignature + "found vertex in range: " + v)

        // Do a type recasting the first time a non void pointer is seen
        if (v.asInstanceOf[DsOliVertexMemory].vType.vType == "void" && vertexType.vType != "void") {
          addNewVertexWithCleanup(vertexStartAddress, vertexEndAddress, vertexType, graph)
        } else {
          v
        }
   ...
   */
  def processMemoryWrite(event: DsOliMWEvent, graph: DsOliGraph): Unit = {
    ...
    if (event.isInstanceOf[DsOliNoAllocEvent]) {
      ...
      //val tVertex = findOrAddVertex(event.rValue.content, targetType, graph)
    ...
    val sourceVertex = findOrAddVertex(contextAddress, sourceType, graph)
  ...
  def removeNodeAndAssociatedEdges(vertex: DsOliVertex, graph: DsOliGraph): Unit = {
    ...
    if (innerVertex != null && innerVertex.incoming != null && innerVertex.incoming.size > 0) {
      DsOliLogger.debug("removeNodeAndAssociatedEdges: innerVertex: " + innerVertex)
      DsOliLogger.debug("removeNodeAndAssociatedEdges: innertVertex.incoming: " + innerVertex.incoming)
      DsOliLogger.debug("removeNodeAndAssociatedEdges: innertVertex.incoming.size: " + innerVertex.incoming.size)
      innerVertex.incoming.foreach {
        innerEdge =>
  ...
  def getEitherVertices(vertex: DsOliVertexMemory, graph: DsOliGraph,
    ...
    while (iter.hasNext) {
      ...
      if (test.isDefined && test.get.asInstanceOf[DsOliVertex] == vertex.asInstanceOf[DsOliVertex]) {
        ...
        DsOliLogger.debug(funSignature + "test != vertex: " + test.get.asInstanceOf[DsOliVertex] + " != " + vertex.asInstanceOf[DsOliVertex])
      }
    }
   ...
   * @return Boolean
   */
  def reachable(vertex: DsOliVertexMemory, Pin: HashMap[Long, DsOliVertexMemory], graph: DsOliGraph): Boolean = {
    val funSignature = classSignature + "reachable: "
    DsOliLogger.debug(funSignature + "entered: ")
    ...
    if (incomingVertices.exists { vertex =>
      ...
      (!leaked.contains(vertex.id)) && DsOliAddressUtils.isStatic(vertex)
    }) {
  ...
  def initCheckLeak(event: DsOliMemoryEvent, ptgIMinusOne: DsOliGraph, ptgI: DsOliGraph) {
    ...
    DsOliLogger.debug(funSignature + "initCheckLeak: leaked = " + leaked)
  }
   ...
   * @param mergedTypeGraph the merged type graph
   */
  def processMemoryEvent(event: DsOliMemoryEvent, graph: DsOliGraph): Unit = {
