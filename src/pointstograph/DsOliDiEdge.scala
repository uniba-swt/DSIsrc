...
class DsOliDiEdge[N](
  ...
  // ! Make sure, that there is only one outgoing edge from a given offset !
  // It is only possible to point from one offset of the source
  // to the target vertex. But it is possible to point from 
  // different offset to the same target vertex address.
  def keyAttributes = Seq(sAddr, sOffset)
