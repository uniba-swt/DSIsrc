...
class DsOliEntryPointCreator(val events: DsOliEvents, val ptgs: DsOliPointsToGraphs, val boxes: DsOliBoxSteps,
  ...
  def retireEPTsForCells(ep: (DsOliVertexMemory, Long), Fdone: DsOliFeatureSet, Fbuilding: DsOliFeatureSet): Unit = {
    ...
    // Copy features from Fbuilding to done
    ...
    // Delete moved features from building
      ...
      if (CupSet.exists {
        cell =>
          ...
          (boxIdSetOld.subsetOf(boxIdSetNew) ||
            ((boxIdSetNew.subsetOf(boxIdSetOld) && (boxIdSetOld.diff(boxIdSetNew).size != 0) && isNotABoxSplit(cell, CupSet, boxIdSetOld, boxIdSetNew, boxesI))) ||
            ((boxIdSetNew.subsetOf(boxIdSetOld) && (boxIdSetOld.diff(boxIdSetNew).size != 0) && cellOffset(targetVertex, cell) == ept.Aup.asInstanceOf[DsOliEPTCell].cellOffset)))
      }) {
        // Calculate the feature
      ...
      event =>
        ...
        if (event.artificialEvents.length != 0) {
            ...
            artificialEvent =>
              ...
              artificialEvent match {
                ...
                case e: DsOliArtificialFreeEvent =>
                  ...
                  //               removeEPTs(e, FdoneArt, FbuildingArt, FsurArt, ptgIMinusOne)
              ...
              val FdoneTmp = new DsOliFeatureSet()
              FdoneTmp.features.appendAll(FdoneArt.features)
              FdoneTmp.features.appendAll(FbuildingArt.features)
              FdoneTmp.features.appendAll(FsurArt.features)

              //DsOliTestMethods.printPTG(ptgI, typeDB, Some(boxesI), null, i, u, FdoneTmp, "_c_event_id_" + event.id + "_art_" + u, true)
          ...
          event match {
            ...
            case e: DsOliVLSEvent =>
              //              removeEPTs(e, Fdone, Fbuilding, Fsur, ptgIMinusOne)
          ...
          val FdoneTmp = new DsOliFeatureSet()
          FdoneTmp.features.appendAll(Fdone.features)
          FdoneTmp.features.appendAll(Fbuilding.features)
          FdoneTmp.features.appendAll(Fsur.features)

          //DsOliTestMethods.printPTG(ptgI, typeDB, Some(boxesI), null, i, 0, FdoneTmp, "_c_event_id_" + event.id, false)
        }
