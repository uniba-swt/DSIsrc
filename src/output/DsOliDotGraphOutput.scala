...
class DsOliDotGraphOutput(typeDB: ITypeDB) extends IDsOliGraphOutput {
   ...
   */
  def deflateType(innerNode: scalax.collection.Graph[DsOliVertex, DsOliDiEdge]#NodeT, vType: DsOliType, baseAddress: Long, sumSizesFromStartAddr: Long, boxes: Option[Set[DsOliBox]], cycles: Option[Set[DsOliCycle]], padding: String = paddingItem): (String, Long) = {
    //DsOliLogger.debug("DsOliDotGraphOutput::deflateType: entered. Processing id: " + innerNode.value.id.toString + ",vType : " + vType + ", baseAddress: " + baseAddress + ", padding: " + padding)
    var rows = ""
    ...
    if (vType != null && vType.fields != null && vType.fields.length != 0) {
      vType.fields.foreach {
        field =>
          //DsOliLogger.debug("DsOliDotGraphOutput::deflateType: inspecting field: " + field)
          ...
          if (tmpSumSizesFromStartAddr < offset) {
            ...
            rows += rowStart +
              createCell(cellData = padding + "compiler padding", colorValue = colorValCompilerPadding) +
          ...
          //DsOliLogger.debug("DsOliDotGraphOutput::deflateType: testing padding: paddingSize: " + paddingSize)
          tmpSumSizesFromStartAddr += paddingSize
          //DsOliLogger.debug("DsOliDotGraphOutput::deflateType: testing padding: tmpSumSizesFromStartAddr: " + tmpSumSizesFromStartAddr.toHexString)
          ...
          if (field.fType == Compound) {
            //DsOliLogger.debug("DsOliDotGraphOutput::deflateType: field is compound: " + field)
            val newType = typeDB.getTypeObject(field.cType)
            if (newType.isDefined) {
              //DsOliLogger.debug("DsOliDotGraphOutput::deflateType: field type is defined. Recurse: " + newType.get + " address: " + baseAddress + field.vOffset)

              val (tmpRows, retSumSizesFromStartAddr) = deflateType(innerNode, newType.get, baseAddress + field.vOffset, tmpSumSizesFromStartAddr, boxes, cycles, padding + paddingItem)
              rows += tmpRows
              tmpSumSizesFromStartAddr = retSumSizesFromStartAddr
            } else {
              //DsOliLogger.warning("DsOliDotGraphOutput::deflateType: unknown type " + field.cType)
            }
          } else {
            //DsOliLogger.debug("DsOliDotGraphOutput::deflateType: testing padding: adding field.vSize: " + field.vSize)
      ...
      //DsOliLogger.debug("DsOliDotGraphOutput::deflateType: just adding to the padding: tmpSumSizesFromStartAddr: " + tmpSumSizesFromStartAddr.toHexString + " vType.size: " + vType.size)
  ...
  def findBoxesAndCyclesForCell(bAddr: Long, eAddr: Long,
    ...
    DsOliLogger.debug("DsOliDotGraphOutput::findBoxesAndCyclesForCell: entered. bAddr: " + bAddr.toHexString + ", eAddr: " + eAddr.toHexString)
    return if (boxes.isDefined && cycles.isDefined) {
      ...
      boxes.get.foreach {
        box =>
          DsOliLogger.debug("DsOliDotGraphOutput::findBoxesAndCyclesForCell: testing box: " + box)
          // Check linear strand part first
          box.cells.foreach {
            cell =>
              DsOliLogger.debug("DsOliDotGraphOutput::findBoxesAndCyclesForCell: cell.bAddr: " + cell.bAddr + ", bAddr: " + bAddr + " cell.eAddr: " + cell.eAddr + ", eAddr: " + eAddr)
              if (cell.bAddr == bAddr && cell.eAddr == eAddr) {
                DsOliLogger.debug("DsOliDotGraphOutput::findBoxesAndCyclesForCell: found box for type: " + box)
          ...
          if (box.cycleId != 0) {
            ...
            if (cycleOpt.isDefined) {
              ...
              DsOliLogger.debug("DsOliDotGraphOutput::findBoxesAndCyclesForCell: no cycle for cycleId: " + box.cycleId)
            }
      ...
      cycles.get.foreach {
        cycle =>
          DsOliLogger.debug("DsOliDotGraphOutput::findBoxesAndCyclesForCell: testing cycle: " + cycle)
          cycle.cells.foreach {
            cell =>
              if (cell.bAddr == bAddr && cell.eAddr == eAddr) {
                DsOliLogger.debug("DsOliDotGraphOutput::findBoxesAndCyclesForCell: found cycle for type: " + cycle)
  ...
  def findBoxesCyclesAndCellsForVertex(vertex: DsOliVertexMemory): Option[(Set[DsOliBox], Set[DsOliCycle], Set[DsOliCell])] = {
    //DsOliLogger.debug("DsOliDotGraphOutput::findBoxesCyclesAndCellsForVertex: entered. vertex: " + vertex)
    return if (this.boxesOpt.isDefined) {
          ...
          box.cells.foreach {
            cell =>
              ...
              if (cell.vertexId == vertex.id) {
                //DsOliLogger.debug("DsOliDotGraphOutput::findBoxesCyclesAndCellsForVertex: found cell for vertex in box: " + cell)
          ...
          if (box.cycleId != 0) {
            ...
            cycle.cells.foreach {
              cell =>
                ...
                // Is the cell associated with the vertex
                if (cell.vertexId == vertex.id) {
                  //DsOliLogger.debug("DsOliDotGraphOutput::findBoxesCyclesAndCellsForVertex: found cell for vertex in cycle: " + cell)
                  // Only record once
                  if (!cellsForVertex.contains(cell)) cellsForVertex += cell
                  if (!boxesForVertex.contains(box)) boxesForVertex += box
                  if (!cyclesForVertex.contains(cycle)) cyclesForVertex += cycle
      ...
      //DsOliLogger.debug("DsOliDotGraphOutput::findBoxesCyclesAndCellsForVertex: box not defined")
      None
    }
  }
          ...
          case v: DsOliVertexMemory =>
            ...
            if (v.vType != null && v.vType.fields != null && v.vType.fields.length != 0) {
              ...
              if (tmpSumSizesFromStartAddr < v.eAddr) {
                ...
                rows += rowStart +
                  createCell(cellData = "compiler padding", colorValue = colorValCompilerPadding) +
                  createCell(cellData = "byte", colorValue = colorValCompilerPadding) +
                  createCell(cellData = paddingSize.toString, colorValue = colorValCompilerPadding) +
