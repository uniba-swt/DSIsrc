...
object DsOliGraphUtils {
  ...
  def getCorrespondingBiDiEdge(edgeBiDi: DsOliMetaBoxGraphDiEdge[DsOliMetaBoxGraphVertex], graph: DsOliMetaBoxGraph): DsOliMetaBoxGraphDiEdge[DsOliMetaBoxGraphVertex] = {
    ...
    val biDiCounterpartSet = graph.graph.edges.filter(edgeBiDiCounter => edgeBiDi.conConf.tag == edgeBiDiCounter.conConf.tag && edgeBiDi.source == edgeBiDiCounter.toOuter.target && edgeBiDi.target == edgeBiDiCounter.toOuter.source && edgeBiDiCounter.conConf.offsets._1 == edgeBiDi.conConf.offsets._2 &&
    ...
    if (biDiCounterpartSet.size == 1) {
      ...
      throw new Exception(funSignature + "No bi directional counterpart found!: " + edgeBiDi.source + "->" + edgeBiDi.target)
    }
  }
  ...
  def isBidirectionalEdge(edge: DsOliMetaBoxGraphDiEdge[DsOliMetaBoxGraphVertex], graph: DsOliMetaBoxGraph): Boolean = {
    ...
    // Must be overlay AND there must exist a corresponding edge where source and target are swapped and the 
    // swapped offsets are matched
    edge.conConf.tag == ccOverlay && graph.graph.edges.exists(edgeBiDi => edgeBiDi.conConf.tag == edge.conConf.tag && edgeBiDi.source == edge.target && edgeBiDi.target == edge.source && edge.conConf.offsets._1 == edgeBiDi.conConf.offsets._2 &&
      edge.conConf.offsets._2 == edgeBiDi.conConf.offsets._1)
  }

  def isBidirectionalEdgeOnSets(edge: DsOliMetaBoxGraphDiEdge[DsOliMetaBoxGraphVertex],
    edges: scala.collection.mutable.Set[DsOliMetaBoxGraphDiEdge[DsOliMetaBoxGraphVertex]]): Boolean = {
    edge.conConf.tag == ccOverlay && edges.exists(edgeBiDi => edgeBiDi.conConf.tag == edge.conConf.tag && edgeBiDi.source == edge.target && edgeBiDi.target == edge.source && edge.conConf.offsets._1 == edgeBiDi.conConf.offsets._2 &&
      edge.conConf.offsets._2 == edgeBiDi.conConf.offsets._1)
  }
  ...
  def conConfClassAgg(conConfClass: scala.collection.mutable.Set[DsOliConConfClassification]): HashMap[DsOliConConfClassificationTag, (Int, Int)] = {
    // Key: label Value: (label count, evidence count)
    ...
    conConfClass.foreach { ccClass =>
      if (ccCount.contains(ccClass.classification)) {
        ccCount.put(ccClass.classification, (ccCount.get(ccClass.classification).get._1 + 1,
          ccCount.get(ccClass.classification).get._2 + ccClass.evidence))
      } else {
    ...
    if (conConfClass != null) {
      ...
      sortedccCount.foreach {
        key =>
          ...
          ret += "<TR><TD ALIGN=\"left\" " + color + ">" + key + 
          "</TD><TD ALIGN=\"left\" " + color + ">" + evidence + " (" + cnt + ")</TD></TR>"
      }
    }
    ret
  }
