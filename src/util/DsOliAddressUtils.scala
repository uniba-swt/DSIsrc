...
object DsOliAddressUtils {
  ...
  def addressInRange(aStartAddress: Long, aEndAddress: Long, bStartAddress: Long, bEndAddress: Long): Boolean = {
    ...
    DsOliLogger.debug(funSignature + "entered: aStartAddress.toHexString = " + aStartAddress.toHexString + "; bStartAddress.toHexString = " + bStartAddress.toHexString)
    DsOliLogger.debug(funSignature + "\taEndAddress.toHexString = " + aEndAddress.toHexString + "; bEndAddress.toHexString = " + bEndAddress.toHexString)
  ...
  def isStatic(vertex: DsOliVertexMemory): Boolean = {
    ...
    // Future work: Needs to be parsed from addresses file!
    val textStartAddress = 0x4008d0.toLong
    val bssEndAddress = 0x606000.toLong

    // Currently only a hard coded observation
    val stackAddressBegin = "7f"
    val stackAddressBegin02 = "bf"
    ...
    isStackAllocated || isGlobalOrStatic
  }
