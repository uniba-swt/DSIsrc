...
{ 
  ...
  logBlockEntry("mbg-sll-with-slls.c", 27);
  tmp = malloc(sizeof(*head));
  logAssignNoOffset("mbg-sll-with-slls.c", 27, & tmp, "void *", "tmp", tmp, "void",
                    "malloc", 1);
  logMallocInfo("sizeof(*head)", sizeof(*head));
  head = (struct SLL *)tmp;
  logAssignNoOffset("mbg-sll-with-slls.c", 27, & head, "struct SLL *", "head", head,
                    "struct SLL", "(struct SLL *)tmp", 0);
  head->next = (struct SLL *)((void *)0);
  logAssignOffset("mbg-sll-with-slls.c", 28, head, "struct SLL", "*head", & head->next,
                  "struct SLL *", "head->next", head->next, "struct SLL", "(struct SLL *)((void *)0)",
                  0);
  iter = head;
  logAssignNoOffset("mbg-sll-with-slls.c", 30, & iter, "struct SLL *", "iter", iter,
                    "struct SLL", "head", 0);
  i = 0;
  logBlockExit("mbg-sll-with-slls.c", 31);
  while (i < 3) {
    logComment("Loop-iteration-start_mbg-sll-with-slls.c_31");
    {
    logBlockEntry("mbg-sll-with-slls.c", 32);
    tmp___0 = malloc(sizeof(*iter));
    logAssignNoOffset("mbg-sll-with-slls.c", 32, & tmp___0, "void *", "tmp___0", tmp___0,
                      "void", "malloc", 1);
    logMallocInfo("sizeof(*iter)", sizeof(*iter));
    iter->next = (struct SLL *)tmp___0;
    logAssignOffset("mbg-sll-with-slls.c", 32, iter, "struct SLL", "*iter", & iter->next,
                    "struct SLL *", "iter->next", iter->next, "struct SLL", "(struct SLL *)tmp___0",
                    0);
    iter = iter->next;
    logAssignNoOffset("mbg-sll-with-slls.c", 33, & iter, "struct SLL *", "iter", iter,
                      "struct SLL", "iter->next", 0);
    iter->next = (struct SLL *)((void *)0);
    logAssignOffset("mbg-sll-with-slls.c", 34, iter, "struct SLL", "*iter", & iter->next,
                    "struct SLL *", "iter->next", iter->next, "struct SLL", "(struct SLL *)((void *)0)",
                    0);
    i ++;
    logBlockExit("mbg-sll-with-slls.c", 31);
    }
  ...
  logBlockEntry("mbg-sll-with-slls.c", 37);
  iter = head;
  logAssignNoOffset("mbg-sll-with-slls.c", 37, & iter, "struct SLL *", "iter", iter,
                    "struct SLL", "head", 0);
  logBlockExit("mbg-sll-with-slls.c", 37);
  while ((unsigned long )iter->next != (unsigned long )((void *)0)) {
    logComment("Loop-iteration-start_mbg-sll-with-slls.c_37");
    {
    logBlockEntry("mbg-sll-with-slls.c", 38);
    tmp___1 = malloc(sizeof(*(iter->child)));
    logAssignNoOffset("mbg-sll-with-slls.c", 38, & tmp___1, "void *", "tmp___1", tmp___1,
                      "void", "malloc", 1);
    logMallocInfo("sizeof(*(iter->child))", sizeof(*(iter->child)));
    iter->child = (struct Child_SLL *)tmp___1;
    logAssignOffset("mbg-sll-with-slls.c", 38, iter, "struct SLL", "*iter", & iter->child,
                    "struct Child_SLL *", "iter->child", iter->child, "struct Child_SLL",
                    "(struct Child_SLL *)tmp___1", 0);
    tmp___2 = malloc(sizeof(*(iter->child)));
    logAssignNoOffset("mbg-sll-with-slls.c", 39, & tmp___2, "void *", "tmp___2", tmp___2,
                      "void", "malloc", 1);
    logMallocInfo("sizeof(*(iter->child))", sizeof(*(iter->child)));
    (iter->child)->next = (struct Child_SLL *)tmp___2;
    logAssignOffset("mbg-sll-with-slls.c", 39, iter->child, "struct Child_SLL", "*(iter->child)",
                    & (iter->child)->next, "struct Child_SLL *", "(iter->child)->next",
                    (iter->child)->next, "struct Child_SLL", "(struct Child_SLL *)tmp___2",
                    0);
    ((iter->child)->next)->next = (struct Child_SLL *)((void *)0);
    logAssignOffset("mbg-sll-with-slls.c", 40, (iter->child)->next, "struct Child_SLL",
